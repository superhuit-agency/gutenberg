...
class Gutenberg_Elements_Test extends WP_UnitTestCase {
	...
	public function test_simple_paragraph_link_color() {
		...
		$this->assertSame(
			$result,
			'<p class="wp-elements-1">Hello <a href="http://www.wordpress.org/">WordPress</a>!</p><style>.wp-elements-1 a{color: var(--wp--preset--color--subtle-background);}</style>' . "\n"
		);
	}
	...
	public function test_class_paragraph_link_color() {
		...
		$this->assertSame(
			$result,
			'<p class="wp-elements-1 has-dark-gray-background-color has-background">Hello <a href="http://www.wordpress.org/">WordPress</a>!</p><style>.wp-elements-1 a{color: red;}</style>' . "\n"
		);
	}
	...
	public function test_anchor_paragraph_link_color() {
		...
		$this->assertSame(
			$result,
			'<p id="anchor" class="wp-elements-1">Hello <a href="http://www.wordpress.org/">WordPress</a>!</p><style>.wp-elements-1 a{color: #fff000;}</style>' . "\n"
		);
	}
}
